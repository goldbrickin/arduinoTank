...
void loop() {
  if (Serial.available()) {
    ...
    if (val == 'd') {
      ...
      getObstacle(getIntSerialParameter());
    } else if (val == 'c') {
      look('c');
    } else if (val == 'l') {
      look('l');
...
boolean getObstacle(int inches)
{
  if (getDistance('i') < inches)
  { 
...
void go(char way, int howFar)
{
  ...
  int lastVisibility = startVisibility;
  int triesWithoutMoving = 0;
  boolean obstacle = false;
  
  while (interruptCounterRight < (howFar * interruptInch))
  {
    if (triesWithoutMoving >= 5)
    {
      Serial.println("obstacle");
      obstacle = true;
      break;
    ...
    if (abs(lastVisibility - startVisibility) <= 2)
      triesWithoutMoving++;
